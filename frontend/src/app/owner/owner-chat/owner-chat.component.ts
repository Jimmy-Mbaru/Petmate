...
export class OwnerChatComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('chatPage') chatPageRef?: ElementRef<HTMLElement>;
  ...
  ngOnInit(): void {
    ...
    this.connectSocket();
    setTimeout(() => this.scrollChatIntoView(), 200);
  }
  ...
  private emitMessagesRead(otherUserId: string): void {
    ...
    // Always call REST fallback to ensure state is synced
    this.chatService.markAsRead(otherUserId).subscribe({
      error: (err) => console.error('Failed to mark messages as read via REST', err)
    });
  }
  ...
  loadConversations(): void {
    ...
    this.chatService.getConversations().subscribe({
      next: (conversations) => {
        ...
        setTimeout(() => this.scrollChatIntoView(), 150);
      },
      error: (error: unknown) => {
        this.isLoading = false;
  ...
  private scrollChatIntoView(): void {
    const el = this.chatPageRef?.nativeElement;
    if (el) {
      ...
      this.socket.emit(
        ...
        (response: unknown) => {
          if (response && typeof response === 'object' && 'error' in response) {
            ...
            const realMsg = response as ChatMessage;
            this.messages = this.messages.map(m => m.id === optimistic.id ? realMsg : m);
          }
        },
      );
    } else {
